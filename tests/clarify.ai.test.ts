...
async function main() {
  ...
  const b = await interpretAnswer("買主様の名義を仲介会社宛てに変えて", fields,
    { buyerName: "前田様個人" }, "北堀江の物件で購入申込書を作成");
  ...
  console.log("\n【B2】部分回答「手付は300万で」");
  const b2 = await interpretAnswer("手付は300万で", fields, {}, "北堀江の物件で購入申込書を作成");
  ...
  for (const t of [
    ...
    const r = await interpretAnswer(t, fields, {}, "北堀江の物件で購入申込書を作成");
  ...
  console.log("\n【B4】履歴の実例「収益」（1語の回答）");
  const b4 = await interpretAnswer("収益", checklistFor("査定書"), {}, "北堀江の査定");
